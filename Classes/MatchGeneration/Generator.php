...
class Generator
{
     ...
     * übergeben werden.
     * Es muss auch im Array $teams enthalten sein!
     * firstmatchday - Spieltag, mit dem die Zählung beginnen soll
     * firstmatchnumber - Spielnummer mit der die Zählung beginnen soll.
     *
     * @param array $teams
     *            sortiertes Array der Liga-Teams. Kann theoretisch von jedem Typ sein. Eine TeamID
     *            wäre aber gut.
     * @param string $table
     *            Spielplan-Tabelle
     * @param array $options
    ...
    private function createTable($teams, $table, $options)
    {
        ...
        if (0 == $option_halfseries || 2 == $option_halfseries) {
            foreach ($table as $day => $matches) {
                $dayArr = []; // Hier kommen die Spiele rein
                foreach ($matches as $k => $match) {
                    $teamIds = explode('-', $match);
    ...
    private function splitTableString($table)
    {
        ...
        if (!strlen(trim($table))) {
            return $ret; // Kein String gesetzt
        }
        $days = explode('|', $table);
