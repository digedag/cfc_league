...
class tx_cfcleague_handleDataInput
{
     ...
     *
     * @param
     *            complete_row wenn false wird nur Name und UID für SELECT-Box geliefert
     */
    public function findTeams($competition, $complete_row = '0')
    {
        ...
        $where = 'FIND_IN_SET(tx_cfcleague_teams.uid,(
            SELECT tx_cfcleague_competition.teams FROM tx_cfcleague_competition
            WHERE tx_cfcleague_competition.uid =' . $competition . '))';

        $res = $GLOBALS['TYPO3_DB']->exec_SELECTquery($what, $from, $where);

        $rows = array();
        while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
            $rows[] = $complete_row ? $row : Array(
                $row['name'],
                $row['uid']
            );
        }
